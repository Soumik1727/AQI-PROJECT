import React from 'react'
import './css/AllParticles.css'
import { useWeather } from '../wrapper/weatherDataWrap'

import pm25 from '../assets/pm25.svg'
import pm10 from '../assets/pm10.svg'
import no2 from '../assets/no2.svg'
import o3 from '../assets/o3.svg'
import co2 from '../assets/co2.png'
import co from '../assets/co.svg'


export default function AllParticles() {
    const { weatherData } = useWeather()
    const particles = [
        { name: 'PM2.5', value: weatherData.pm2_5, unit: 'µg/m³', img: pm25, full_name: 'Particulate Matter ' },
        { name: 'PM10', value: weatherData.pm10, unit: 'µg/m³', img: pm10, full_name: 'Particulate Matter ' },
        { name: 'NO2', value: weatherData.no2, unit: 'µg/m³', img: no2, full_name: 'Nitrogen Dioxide ' },
        { name: 'O3', value: weatherData.o3, unit: 'µg/m³', img: o3, full_name: 'Ozone ' },
        { name: 'CO2', value: weatherData.co2, unit: 'µg/m³', img: co2, full_name: 'Carbon Dioxide' },
        { name: 'CO', value: weatherData.co, unit: 'µg/m³', img: co, full_name: 'Carbon Monoxide ' },
    ]

    return (
        <div className='allps'>
            {particles.map((particle, index) => (
                <div className="pollutent" key={index}>
                    <img src={particle.img} alt="" />
                    <div className='name'>{particle.full_name} <br /> ({particle.name})</div>
                    <div className="amount">
                        <div className="value">{particle.value}</div>
                        <div className="unit">{particle.unit}</div>
                    </div>
                </div>
            ))}
        </div>
    )
}
